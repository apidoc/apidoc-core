...
function isRequired(schema, key) {
	...
	console.log('required', key, schema);
	var objectHasRequired = exists(Object.keys(schema),'required') && (schema.required.indexOf(key) !== -1);
	var itemsHasRequired = exists(Object.keys(schema),'items') && exists(Object.keys(schema.items),'required') && (schema.items.required.indexOf(key) !== -1);
	
	return ( objectHasRequired || itemsHasRequired );
}
...
function mergeAllOf(schema) {
	if (exists(Object.keys(schema),'allOf')) {
		for(var i = schema.allOf.length; i--;) {
			...
			console.log('merge', schema, schema.allOf[i]);
