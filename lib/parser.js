...
function Parser(_app) {
    ...
    // load schema
    var schemas = Object.keys(app.schemas);
    schemas.forEach(function(schema) {
        var filename = app.schemas[schema];
        app.log.debug('load schema: ' + schema + ', ' + filename);
        self.addSchema(schema, require(filename));
    });
}
 ...
 * Add a Schema
 */
Parser.prototype.addSchema = function(name, schema) {
    ...
    while (matches) {
        ...
        if (element.name === 'apischema') {
            ...
            if (self.schemas[values.schema]) {
            	var blockExtend = self.schemas[values.schema](values.type, JSON.parse(schema));
