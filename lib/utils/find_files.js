...
/**
 * Get Directory files recursivly
 *
 * @returns {String[]}
 */
function recursiveReaddirSync(root) {
  var list = [];
  ...
  files.forEach(function (file) {
    ...
    if (stats.isDirectory()) {
      list = list.concat(recursiveReaddirSync(fullPath));
    } else {
      list.push(fullPath);
    }
  });
...
FindFiles.prototype.search = function() {
    ...
    try {
        // find Files
        files = recursiveReaddirSync(self.path).map(function(val) {
          return path.relative(self.path, val);
        });
